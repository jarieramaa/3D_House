...
class Data:
    """Data class keeps all the information about all the things that are got via API.
    The idea is that this data is saved to the file and next time the same address is
    searched, the data is loaded from the file (not via API). That way, the application
    is a lot faster."""
    ...
    def __init__(self, window) -> None:
        ...
        self._post_code = ""
        self._municipality = (
            ""  # Municipality, this field could be added to the search query
        )
    ...
    def post_code(self, value: str) -> None:
        ...
        if value is not None and (len(value) != 4 and len(value) != 0):
            self._validated = False
